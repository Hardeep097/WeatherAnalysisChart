...
namespace Tests_WeatherData
{
    ...
    {
        private static string filename = @"C:\Users\hsingh\Desktop\C# Revision\Ex_Files_Code_Clinic_C_Sharp\Exercise Files\Ch01\pond_data\Environmental_Data_Deep_Moor_2012.txt";
...
2012_01_01 00:21:00	33.80	30.60	27.30	76.60	280.80	17.00	14.00";
        ...
        public void Test_060_FitLineToFilteredSampleFile()
        {
            ...
            using (var text = new StreamReader(filename))
            {
                ...
                var data = from wo in WeatherData.ReadRange(text, start, end) // extratct
                           select new //transforming
                           {
                               Hour = (wo.TimeStamp - start).TotalHours,
             ...
             var num =     MathNet.Numerics.Fit.Line(arrX.ToArray(), arrY.ToArray());

                Check.That(num.Item2).IsLessThan(0);
